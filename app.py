...
def main():
    ...
    if cap is None or not cap.isOpened():
        print("No camera found! Creating demo mode with colored frames...")
        # Create demo mode with synthetic frames
        demo_mode = True
    else:
        demo_mode = False
    ...
    try:
        while True:
            if demo_mode:
                ...
                frame[:, :, 0] = np.random.randint(0, 255, size=(480, 640))  # Random blue channel
                frame[:, :, 1] = np.random.randint(0, 255, size=(480, 640))  # Random green channel
                frame[:, :, 2] = np.random.randint(0, 255, size=(480, 640))  # Random red channel
            else:
                ret, frame = cap.read()
                if not ret:
                    break
